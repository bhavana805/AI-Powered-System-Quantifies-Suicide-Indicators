...
global le
main = tkinter.Tk()
main.title("AI-Powered System Quantifies Suicide Indicators and Identifies Suicide Related Content in Online Posts") #designing main screen
...
def translation():
    ...
    Y = dataset.values[:,2]
    cols =['gender','sexuallity','race','bodyweight','virgin','prostitution_legal','pay_for_sex','social_fear','depressed','what_help_from_others','attempt_suicide','employment','job_title','edu_level','improve_yourself_how']
    le = LabelEncoder()
...
def trainCNN():
    ...
    classifier.add(Convolution2D(32, 1, 1, input_shape = (X_train1.shape[1],
...
def predict():
    ...
    cols =['gender','sexuallity','race','bodyweight','virgin','prostitution_legal','pay_for_sex','social_fear','depressed','what_help_from_others','employment','job_title','edu_level','improve_yourself_how']
...
def graph():
    df =pd.DataFrame([['CNN','Precision',precision[0]],['CNN','Recall',recall[0]],['CNN','F1 Score',fscore[0]],['CNN','Accuracy',accuracy[0]],['Random Forest','Precision',precision[1]],['RandomForest','Recall',recall[1]],['Random Forest','F1 Score',fscore[1]],['RandomForest','Accuracy',accuracy[1]],],columns=['Parameters','Algorithms','Value'])
